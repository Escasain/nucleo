...
export const FUND_PROG_PRACTICE = [
  ...
  // ---------- Métodos
  {
    ...
    a: 'static int maximo(int a, int b, int c) {\n    int mayor = a;\n    if (b > mayor) mayor = b;\n    if (c > mayor) mayor = c;\n    return mayor;\n}\n\nLa idea es la del candidato: empiezas suponiendo que gana el primero y lo sustituyes cada vez que aparece uno mejor. Es el mismo esquema que usarás para buscar el máximo de un array, solo que allí el bucle recorre los demás.\n\nCon ifs encadenados también sale, pero se lía enseguida:\n\n    if (a >= b && a >= c) return a;\n    else if (b >= c) return b;\n    else return c;\n\nLos >= no son decorativos, pero tampoco pasa lo que mucha gente supone: con > estrictos esto COMPILA igual, porque el else final siempre devuelve algo, y con tres números iguales devuelve c — que es la respuesta correcta, porque son todos el mismo número.\n\nLo que cambia es que ya no puedes saberlo de un vistazo: hay que ir caso por caso comprobando que ninguna combinación se cuela por la rama equivocada. El esquema del candidato no tiene ramas que revisar, y por eso es el que se recomienda.'
  },
  {
    id: 'fm2',
    g: 'Métodos',
