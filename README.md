...
- **Conceptos clave y trampas** — dentro de la guía de cada asignatura: el vocabulario que bloquea, definido sin usar el propio término, con para qué sirve y con qué se confunde; y «dónde se cae todo el mundo», los errores que se repiten examen tras examen con lo que se hace mal y lo que es. Cada concepto se convierte en tarjeta de repaso de un clic, o todos de golpe: ahí estaba el verdadero coste de las flashcards. Disponible en Álgebra y Matemática Discreta, Tecnología de Computadores y Fundamentos de Programación (136 conceptos y 36 trampas); añadir otra asignatura es crear su fichero en `src/data/concepts/`.
- **Práctica con solución razonada** — una pestaña por asignatura con problemas para resolver en papel: 28 en Álgebra, 21 en Tecnología de Computadores y 29 en Fundamentos de Programación, por tema y con su nivel (básico, medio, examen). La pista empuja sin dar la respuesta y la solución viene paso a paso, las dos detrás de un clic para que el ejercicio no se convierta en lectura. Te autocorriges, los fallados se filtran aparte para volver sobre ellos, y el panel te dice en qué tema flojeas de verdad. Al fallar puedes apuntar qué te faltaba y pasa a tus dudas.
- **Cómo se hace** — el escalón que falta entre entender el concepto y saber resolver el problema: la receta. Una pestaña por asignatura con los procedimientos que de verdad se piden — inducción, Gauss, Bézout, complemento a 2, Karnaugh, secuenciales, caché, trazar código a mano, escribir una clase — cada uno con **cuándo se usa** (reconocer qué método pide un enunciado es la mitad del examen), los pasos con su nota de dónde se tuerce cada uno, la trampa que se come a media clase y un ejemplo resuelto detrás de un clic. Se ordenan por lo que estás fallando en Práctica: si fallas tres problemas de inducción, lo que te falta no es teoría, es el método. 29 procedimientos en las tres asignaturas con problemas; añadir otra es crear su fichero en `src/data/procedures/`.
- **Mapa de temas** — el temario de una asignatura dibujado como lo que es: un grafo de dependencias. Qué hay que llevar antes de meterse con qué, y qué aporta exactamente cada flecha. Cruza el mapa con lo que llevas hecho y con tus problemas fallados, así que marca en rojo el tema que se te está resistiendo y te dice por dónde seguir — dando prioridad a lo que está más arriba, porque un tema flojo arrastra a todos los que se apoyan en él. Un tema con el temario terminado pero sin problemas resueltos no cuenta como sólido: haber leído no es haber comprobado.
- **Simulacro de examen** — monta un examen cronometrado con tus propios problemas, pesando el nivel y dando preferencia a lo que aún no tienes limpio. Se corrige al terminar, con nota y desglose por tema. El reloj cuenta tiempo de reloj, no ticks, así que cambiar de pestaña no te regala minutos.
- **Ficha de repaso imprimible** — una hoja por asignatura para los días de antes: tus temas flojos, las trampas que encajan con ellos, los problemas fallados con su respuesta corta, tus dudas resueltas y el vocabulario. Pensada para leerse en papel tanto como en pantalla.
- **Conexiones entre asignaturas** — la misma idea vista desde varias asignaturas: la lógica de Álgebra, las puertas de Tecnología y el `if` de Java son el mismo álgebra escrita de tres maneras. Cada puente dice qué se traslada tal cual de una a otra y, sobre todo, **dónde deja de valer el parecido** — que es la parte que nadie escribe y la que cuesta un examen. Vista propia a lo ancho del grado y una pestaña en cada asignatura.
